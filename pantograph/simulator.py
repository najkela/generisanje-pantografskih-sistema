...
def circle_intersect(
    ...
    Враћа `None` и ако је производ полуга нула (`r1 * r2 == 0.0`,
    docs/NALAZ_15_09_nulta_poluga.md, допуна 16.09.) — не провера сваке полуге понаособ.
    Полуге настају градуалним смањивањем, не скоком (crossover/mutate_coords дрифт,
    исти налаз §4: нађена инстанца ивице дужине ~1e-15), па је сасвим могуће да ОБЕ полуге
    истог троугла истовремено падну довољно ниско да њихов производ подбаци на `0.0`
    (IEEE754 underflow) док је свака појединачно још формално ненулта — провера само на
    појединачну нулу то не хвата, а исти `ZeroDivisionError` пролази кроз њу. Круг
    полупречника нула (или занемарљивог производа) нема праву кружницу пресека, третира се
    исто као „пресека нема". Провера иде ПРЕ позива `circle_intersect_pair` — не само
    јефтино прво/скупо после, него зато што се на `circle_intersect_pair`-ову сопствену
    проверу опсега не може ослонити: за `r1==0` и `d==r2` тачно (или `r2==0` и `d==r1`
    тачно), та функција ипак враћа ВАЖЕЋИ пар (`h=0`) — управо тај гранични случај је узрок
    стварног `ZeroDivisionError`-а на `sin_angle` испод (мерено: `baseline egg seed2` и
    `baseline superellipse seed2`, ноћно покретање 15.09.).
...
def positions_and_angle_at(
    ...
    хеуристике по близини. Иста провера као `circle_intersect` — производ полуга
    `ra * rb == 0.0`, не појединачна нула (docs/NALAZ_15_09_nulta_poluga.md, допуна
    16.09.: полуге се сливају ка нули, па обе могу истовремено пасти довољно ниско да
    производ подбаци пре него што иједна погоди тачну нулу) — мора мерити исти механизам
    као `simulate` (DECISIONS §17), па не сме бити блажа од ње.
    """
    positions = base.copy()
    r02 = _link_length(lengths, FIXED_A, CRANK)
    positions[CRANK] = positions[FIXED_A] + r02 * np.array([np.cos(angle), np.sin(angle)])
